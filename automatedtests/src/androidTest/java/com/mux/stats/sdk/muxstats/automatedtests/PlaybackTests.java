...
public class PlaybackTests extends TestBase {
  ...
  public void testVideoChange(boolean programChange, boolean videoChange) {
    try {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          testActivity.setUrlToPlay(secondVideoToPlayUrl, SourceType.Hls);
          CustomerVideoData customerVideoData = new CustomerVideoData();
          customerVideoData.setVideoTitle(BuildConfig.FLAVOR + "-" + currentTestName.getMethodName()
              + "_title_2");
          MuxStatsSDKBitmovinPlayer muxStats = testActivity.getMuxStats();
          if (videoChange) {
  ...
  public void testEndEvents() {
    try {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          double contentDuration = pView.getPlayer().getDuration();
          pView.getPlayer().seek(contentDuration - 2);
        }
      });
  ...
  public void testVodPlayback() {
    try {
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          double currentPlaybackPosition = pView.getPlayer().getCurrentTime();
          pView.getPlayer().seek(currentPlaybackPosition / 2);
        }
      });
      ...
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          double currentPlaybackPosition = pView.getPlayer()
              .getCurrentTime();
          double videoDuration = pView.getPlayer().getDuration();
          double seekToInFuture =
              currentPlaybackPosition + ((videoDuration - currentPlaybackPosition) / 2);
          pView.getPlayer().seek(seekToInFuture);
        }
      });
      Thread.sleep(PLAY_PERIOD_IN_MS);
      testActivity.runOnUiThread(new Runnable() {
        public void run() {
          pView.getPlayer().pause();
        }
      });
